...
export default async function handler(req, res) {
  ...
  let inserted;
  try {
    inserted = await insertLead({ supabaseUrl, serviceRoleKey, lead });
  } catch (error) {
    console.error('estimate_supabase_error', escapeHtml(error.message || String(error)));
    return sendJson(res, 500, { error: 'Submission failed.' });
  }

  // Lead is captured in Supabase — the submission is already successful.
  // Email notification is best-effort and must never turn a saved lead into a customer-facing error.
  if (!gmailToken) {
    console.log('email_notification_not_configured: GMAIL_ACCESS_TOKEN is absent; skipping notification to ' + TEST_RECIPIENT);
  } else {
    try {
      await sendOwnerEmail({ gmailToken, lead, createdAt: inserted.created_at || Date.now() });
    } catch (error) {
      console.error('email_notification_failed', escapeHtml(error.message || String(error)));
    }
  }
